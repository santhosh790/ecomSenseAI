"""
Test with COMPLETE real PDF extraction from user.
"""
...
# Complete real PDF text from user
complete_pdf_text = """Purchase Order
Vendor Code
405485
Vendor Name
PKS FRESH
...
print("=" * 80)
print("VIT Extraction Test with COMPLETE Real PDF")
print("=" * 80)

results, report = detect_vegetables(
    text=complete_pdf_text,
...
print(f"  VIT Mode: {report.get('vit_mode_activated', False)}")
...
missing = [v for v in expected if not any(v.lower() in r['Source Name'].lower() for r in results)]
if missing:
